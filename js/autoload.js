try {
    $("<link>").attr({href: "https://imgs.sakikency.top/assets/waifu.css", rel: "stylesheet", type: "text/css"}).appendTo('head');
    $('body').append(
    ...
    $.ajax({url: 'https://imgs.sakikency.top/assets/waifu-tips.min.js',dataType:"script", cache: true, async: false});
    $.ajax({url: 'https://imgs.sakikency.top/assets/live2d.min.js',dataType:"script", cache: true, async: false});
    /* 可直接修改部分参数 */
    ...
    /* 在 initModel 前添加 */
    initModel('https://imgs.sakikency.top/assets/waifu-tips.json');
